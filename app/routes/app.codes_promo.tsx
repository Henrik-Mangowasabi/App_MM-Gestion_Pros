...
export default function CodesPromoPage() {
  ...
  const currentEntries = (entries as any[]).slice(
      ...
      <div style={{ maxWidth: containerMaxWidth, margin: "0 auto" }}>
        <details style={styles.infoDetails}>
          <summary style={styles.infoSummary}>
            ℹ️ Guide d&apos;activation (Cliquez pour dérouler)
          </summary>
          <div
            style={{
          ...
          >
            <p style={{ marginTop: 0 }}>
              <strong>Contrôle des réductions :</strong>
            </p>
            <ul style={{ paddingLeft: "20px", margin: "10px 0" }}>
              <li>
                <strong>État Actif :</strong> Le code promo est utilisable
                immédiatement.
              </li>
              <li>
                <strong>État Inactif :</strong> Le code est désactivé
                temporairement.
              </li>
            </ul>
          </div>
      ...
      <div style={{ maxWidth: containerMaxWidth, margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            >
              Liste des Codes Promo ({entries.length})
            </h2>
          </div>
                ...
                {currentEntries.length === 0 ? (
                  ...
                  currentEntries.map((entry: any, index: number) => {
                    const isUpdatingThis =
                      fetcher.formData?.get("id") === entry.id &&
                      fetcher.state !== "idle";
                    ...
                    return (
                      <tr key={entry.id}>
                        ...
                        >
                          {entry.discount_id && (
                            <fetcher.Form method="post">
                              ...
                              <button
                                type="submit"
                                disabled={isUpdatingThis || isLocked}
                                style={{
                                  padding: "6px 10px",
                                  backgroundColor:
                                    isUpdatingThis || isLocked
                                      ? "#f4f6f8"
                                      : isActive
                                        ? "white"
                                        : "#008060",
                        ...
                        >
                          {entry.discount_id ? (
                            <a
                              href={`shopify:admin/discounts/${extractId(entry.discount_id)}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              style={styles.adminBtn}
                              title="Voir dans Shopify"
                            >
                              ↗
                            </a>
                          ) : (
